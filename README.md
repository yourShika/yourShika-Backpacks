...
[![Version](https://img.shields.io/badge/version-1.5.1-6E5BC8)](https://github.com/yourShika/yourShika-Backpacks/releases)
...
- 🛟 **Fixed a rare item-loss race in the storage cache** – with the async write-behind
  cache (v1.2.0), a backpack save on the main thread (e.g. a Pickup upgrade depositing a
  freshly-picked-up item) could run *while* the background flush was serialising the very
  same live data. That could tear the write, and — worse — the flush clearing the "dirty"
  flag could drop a save that happened during it, so an item occasionally **vanished**
  (e.g. noticed while rerolling a villager: breaking/replacing a Lectern repeatedly, one
  would go missing every so often). The cache now snapshots a **deep copy on the main
  thread at save time** and only ever flushes that immutable snapshot, and re-arms the
  dirty flag correctly, so concurrent saves can no longer be lost or corrupted. Affects
  **all** backpack writes (pickup, furnace, XP, upgrades …), not just Lecterns.
  - *Note:* if a Pickup / Advanced-Pickup upgrade is vacuuming items you'd rather keep in
    hand, set a **Pickup filter** to whitelist only what you want, or toggle it off with
    `/bp magnet off` — the items were going into the backpack (this fix stops them being lost).
...
- 📖 **`/bp info` recipe browser now shows the Dragon items** – the overview backpack
  and upgrade rows grew (they were capped at 7 each) so the **Dragon Backpack**, the
  **Dragon Upgrade** and the **Dragon Core** all appear. Clicking the Dragon Upgrade now
  shows its real **multi-stage** recipe (Dragon Core + Upgrade Leather + Dragon Breath)
  instead of a wrong generic "8× material" grid, and the **Dragon Core** has its own
  recipe view (End materials). The final Netherite → Dragon smithing step was already
  shown correctly. Recipe grids read straight from `upgrades.dragon` config.
...
- ♻️ **`/bp restore <player> <id>`** *(new, admin)* – hands a **lost** backpack back
  to a player. Because a backpack's contents live server-side by ID, a destroyed or
  despawned backpack item can be rebuilt: the command recreates the item from the
  stored data (tier, colour, owner, name) with the **same ID**, so all contents are
  intact. It creates no new data and never changes the contents; find the ID with
  `/bp list <player>`. Node `yourshika.backpack.admin.restore`.
...
- 🎨 **`item-model` / `custom-model-data` on upgrade items now work without Oraxen**
  ([#8](https://github.com/yourShika/yourShika-Backpacks/issues/8)) – the
  `upgrades.models.*` and `upgrades.functions.models.*` `item-model` option was only
  applied while an external item hook (Oraxen) was active, so on a plain
  resource-pack setup the upgrades always stayed `minecraft:paper` / `minecraft:leather`.
  These are vanilla components, so they are now applied straight from the config
  regardless of Oraxen — matching how backpack tier items already behaved. Set e.g.
  `item-model: "minecraft:diamond"` (or your own pack model) and it takes effect. An
  empty `item-model` still means "plain vanilla item", and an active Oraxen hook still
  overrides via `provider-id` as before.
...
- 🇵🇱 **Polish translation completed** – the Polish locale was missing 25 keys
  (whole `rename`, `transfer`, `locate`, `assets`, `doctor`, `trash` sections plus
  some help/error entries) that fell back to English. All three bundled locales
  (`en`, `de`, `pl`) are now at **full key parity (102/102)** – no more English
  fallback for Polish players.

---

## 🌐 v1.2.1

- 🌐 **All languages ready via config** – `en`, `de` and `pl` are now **all extracted
  to the data folder on startup**, so you can switch `language:` to any of them and
  edit any `messages_<lang>.yml` directly (previously only the active language and
  English were written out).
- 🛡️ **Translations survive updates** – on every start each bundled language file is
  **merged**: new message keys are added while your own edits are kept untouched, so
  ...
  language by creating `messages_<code>.yml` and setting `language:` to that code.
...
- 🔄 **Textures update after `/oraxen reload`** – after redeploying assets and
  reloading Oraxen, freshly added/changed textures didn't apply to crafted, given
  or `/bp info` items (the plugin still served its cached item models) until you
  toggled the module off and on. The plugin now listens for Oraxen's reload event
  and **auto-resyncs** its canonical items, carried backpacks and placed models, so
  new textures apply immediately – no module toggle needed.
- 🧍 **Placed models follow the module state** – disabling the Oraxen module (or
  removing Oraxen entirely) left placed backpack 3D models in the world, which would
  show missing textures once Oraxen is gone. Placed models now **fall back to the
  vanilla look** when the hook is off – updated on toggle/reload for loaded ones and
...
```
target/yourShika-Backpacks-1.5.1.jar
```
