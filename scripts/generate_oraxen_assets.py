...
TIER = {
    ...
    "dragon": ("#2b1b45", "#c56bff", 1008, "Dragon-Rucksack"),
}
...
def bundle_overlay_layer(tier_key=None):
    # WICHTIG: Das Overlay ist die oberste Ebene des getragenen Modells. Es darf
    # deshalb KEINE Tier-Farbe einbacken – sonst überdeckt es die über die
    # Lederfarbe getönte Base (Hauptfarbe) und den getauschten Akzent, und jeder
    # Rucksack behält seine Tier-Farbe (z.B. Emerald bleibt grün). Das Overlay ist
    # daher tier-unabhängig und rein neutral (nur Naht/Schatten-Detail), analog zur
    # neutralen Base und zur v1.0.9-Korrektur der platzierten Modelle.
    src = bundle_source()
    out = Image.new("RGBA", (16, 16), ALPHA)
    stitch = (54, 47, 42, 255)  # neutrales dunkles Naht-/Schattendetail
    for y in range(16):
        for x in range(16):
            color = src.getpixel((x, y))
            if color[3] == 0:
            ...
            if is_gray:
                ...
                out.putpixel((x, y), stitch)
    # Kleine, neutrale Schnalle (nicht tier-/akzentgefärbt).
    px(out, 7, 9, DARK_IRON)
    px(out, 8, 9, IRON)
    return out
...
FUNCTIONS = [
    ...
    ("soulbound", "Soulbound Upgrade", "#7ce0ff", "soulbound", False, 2156),
    ...
    ("pet_booster", "Pet Booster Upgrade", "#e040fb", "pet_booster", False, 2157),
]
...
def draw_upgrade_tiers():
    ...
    for tier in ("copper", "iron", "gold", "diamond", "emerald", "netherite", "dragon"):
        img = icon_canvas()
        d = ImageDraw.Draw(img)
        color = c(TIER[tier][1] if tier == "dragon" else TIER[tier][0])
...
def draw_dragon_core():
    # End-Kristall-artiges Icon (violett) für das Dragon-Core-Zwischenprodukt.
    img = icon_canvas()
    d = ImageDraw.Draw(img)
    core = c("#c56bff")
    dark = c("#7a3bff")
    glow = c("#ecd6ff")
    poly(d, [(8, 1), (13, 8), (8, 15), (3, 8)], OUTLINE)
    poly(d, [(8, 2), (12, 8), (8, 14), (4, 8)], dark)
    poly(d, [(8, 3), (11, 8), (8, 12), (5, 8)], core)
    line(d, [(8, 4), (8, 12)], lighten(core, 0.30))
    line(d, [(5, 8), (11, 8)], lighten(core, 0.20))
    px(img, 8, 8, glow)
    px(img, 8, 7, glow)
    px(img, 8, 0, glow)
    px(img, 2, 8, glow)
    px(img, 14, 8, glow)
    save_icon(img, TEXTURES / "upgrades/dragon_core.png")
...
def glyph_soulbound16(d, color):
    # Eine Seelenflamme (Soulbound) über einem bindenden Ring – "an die Seele
    # gebunden, überlebt den Tod".
    inner = c("#e8fbff")
    # Flammen-Silhouette
    poly(d, [(8, 2), (11, 6), (11, 10), (8, 12), (5, 10), (5, 6)], OUTLINE)
    poly(d, [(8, 4), (10, 7), (9, 10), (7, 10), (6, 7)], color)
    line(d, [(8, 5), (8, 9)], inner)
    d.point((8, 6), fill=inner)
    # Bindender Ring/Glied darunter
    rect(d, (6, 12, 10, 13), IRON)
    rect(d, (7, 12, 9, 12), DARK_IRON)
    d.point((6, 13), fill=color)
    d.point((10, 13), fill=color)
...
def draw_function(slug, color, kind, advanced):
    ...
    elif kind == "pet_booster":
        glyph_xp16(d, col)
        # kleiner Aufwärts-"Boost"-Pfeil oben rechts.
        px(img, 12, 3, lighten(col, 0.4))
        px(img, 11, 4, lighten(col, 0.4))
        px(img, 13, 4, lighten(col, 0.4))
        px(img, 12, 4, col)
        px(img, 12, 5, col)
    elif kind == "battery":
...
def write_core_yaml():
    ...
    upgrades = [
        ...
        ("dragon", "Dragon-Upgrade", "#C56BFF", "PAPER", 2007),
        ("dragon_core", "Dragon Core", "#C56BFF", "PAPER", 2160),
    ]
